...
ecc_param_pem="../ecc/secp256r1.pem"

echo "Prepare ECC TLS credentials (Client side material)"

if [ ! -e ${ecc_param_pem} ]; then
	echo "Creating ECC parameter file: ${ecc_param_pem} for secp256r1"
	openssl ecparam -name secp256r1 -out ${ecc_param_pem}
else
...
if [ ! -e ${rootca_key} ]; then
	...
	openssl req -x509 -new -nodes -key ${rootca_key} \
	  -subj "/O=NXP-Demo-CA/OU=Demo-Unit/CN=demoCA" \
...
echo ">> Now create CSR"
openssl req -new -key ${client_key} -subj "/O=NXPDemo/OU=Unit/CN=ECC-demo-client" -out ${client_csr}
...
if [ ! -e ${server_key}  ]; then
	...
	openssl req -new -key ${server_key} -subj "/O=NXPDemo/OU=Unit/CN=ECC-demo-server" -out ${server_csr}
else
